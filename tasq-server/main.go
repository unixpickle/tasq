...
func (s *Server) ServeSummary(w http.ResponseWriter, r *http.Request) {
	...
	err := s.Queues.Iterate(func(name string, qs *QueueState) {
		...
		counts := qs.Counts(0, false, true, true)
...
func (s *Server) ServeCounts(w http.ResponseWriter, r *http.Request) {
	...
	includeFailed := r.URL.Query().Get("includeFailed") == "1"
	...
	if r.URL.Query().Get("all") == "1" {
		...
		err := s.Queues.Iterate(func(name string, qs *QueueState) {
			...
			allCounts = append(allCounts, qs.Counts(rateWindow, includeModtime, includeBytes, includeFailed))
		})
		if err != nil {
	...
	err := s.Queues.Get(r.URL.Query().Get("context"), func(qs *QueueState) {
		obj = qs.Counts(rateWindow, includeModtime, includeBytes, includeFailed)
	})
	if err != nil {
		serveError(w, err.Error(), http.StatusServiceUnavailable)
	} else {
...
func (s *Server) ServePopTask(w http.ResponseWriter, r *http.Request) {
	...
	err := s.Queues.Get(r.URL.Query().Get("context"), func(qs *QueueState) {
		...
		task = NewTaskResponse(popped, includeAttempts, true)
	})
	if err != nil {
		serveError(w, err.Error(), http.StatusServiceUnavailable)
	} else if task != nil {
...
func (s *Server) ServePopBatch(w http.ResponseWriter, r *http.Request) {
	...
	result["tasks"] = NewTaskResponses(tasks, includeAttempts, true)
...
func (s *Server) ServePeekTask(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		serveObject(w, NewTaskResponse(task, includeAttempts, false))
	} else {
		if nextTask != nil {
			...
			serveObject(w, map[string]interface{}{
				"done":  false,
				"retry": math.Max(0, timeout.Seconds()),
				"next":  NewTaskResponse(nextTask, includeAttempts, false),
			})
		} else {
			serveObject(w, map[string]interface{}{"done": true})
		}
...
func NewTaskResponse(task *Task, includeAttempts, pop bool) *TaskResponse {
	if task == nil {
		return nil
	}
	var attempts *int64
...
func NewTaskResponses(tasks []*Task, includeAttempts, pop bool) []*TaskResponse {
	responseTasks := make([]*TaskResponse, 0, len(tasks))
	for _, task := range tasks {
		responseTasks = append(responseTasks, NewTaskResponse(task, includeAttempts, pop))
	}
